    ...
    stages {
        stage('SCM Checkout') {
            steps {
                ...
                git 'https://github.com/iamajaygaur/insurance.git'
            }
		}
        stage('Maven Build') {
            steps {
       ...
       stage("Docker build"){
            steps {
				sh 'docker version'
				sh "docker build -t iamajaygaur/insurance:${BUILD_NUMBER} ."
				sh 'docker image list'
				sh "docker tag iamajaygaur/insurance:${BUILD_NUMBER} iamajaygaur/insurance:latest"
            }
        }
		...
		stage('Push2DockerHub') {
			...
			steps {
				sh "docker push iamajaygaur/insurance:latest"
			}
		}
